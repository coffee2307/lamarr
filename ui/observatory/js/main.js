/**
 * LAMARR Observatory — CSI field visualization
 * Live mode: 3D voxel heatmap from signal_field (no procedural skeleton).
 * Demo mode: optional scenario playback for testing without hardware.
 */
...
import { PresenceCartography } from './presence-cartography.js';
...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('lamarr-settings-version')
        || localStorage.getItem('ruview-settings-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('lamarr-observatory-settings')
          || localStorage.getItem('ruview-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('lamarr-observatory-settings');
        localStorage.setItem('lamarr-settings-version', SETTINGS_VERSION);
      }
    } catch {}
    ...
    this._presenceCartography = new PresenceCartography(this._scene);
    this._presenceCartography.group.visible = false;
  ...
  _wsCandidatesFromHttpBase(base) {
    const urlObj = new URL(base);
    const proto = base.startsWith('https') ? 'wss:' : 'ws:';
    const host = urlObj.hostname;
    const port = urlObj.port || (base.startsWith('https') ? '443' : '80');
    const out = [];
    if (port === '3000') out.push(`${proto}//${host}:3001/ws/sensing`);
    out.push(`${proto}//${urlObj.host}/ws/sensing`);
    if (port === '8080') out.push(`${proto}//${host}:8765/ws/sensing`);
    return [...new Set(out)];
  }
  ...
  _autoDetectLive() {
    ...
    const tryNext = (i) => {
      if (i >= unique.length) {
        console.log('[Observatory] No sensing server — demo mode (mô phỏng)');
        this.settings.dataSource = 'demo';
        this._hud.updateSourceBadge('demo', null);
        this._hud.setWaitBanner(false, '');
        return;
      }
      const base = unique[i];
      fetch(`${base}/health`, { signal: AbortSignal.timeout(1500) })
        ...
        .then(data => {
          if (data && data.status === 'ok') {
            console.log('[Observatory] Server at', base, '— chờ CSI thật, không chạy demo');
            this.settings.dataSource = 'ws';
            this._demoData._autoMode = false;
            this._demoData.setScenario('empty_room');
            this._liveFrameCount = 0;
            this._hud.setScenarioLocked(true);
            this._connectWSList(this._wsCandidatesFromHttpBase(base));
          } else {
            tryNext(i + 1);
          }
        })
  ...
  _connectWSList(urls, idx = 0) {
    if (idx >= urls.length) {
      console.warn('[Observatory] WebSocket failed on all ports — demo mode');
      this.settings.dataSource = 'demo';
      this._hud.updateSourceBadge('demo', null);
      return;
    }
    this._connectWS(urls[idx], () => this._connectWSList(urls, idx + 1));
  }

  _connectWS(url, onFail) {
    this._disconnectWS();
    try {
      ...
      this._ws.onopen = () => {
        ...
        this._hud.updateSourceBadge('wait', this._ws);
        this._hud.setWaitBanner(true, 'Đã kết nối server — chờ khung CSI từ ESP32 (UDP :5005). Không hiển thị người giả.');
      };
      ...
      this._ws.onclose = () => {
        ...
        this.settings.dataSource = 'demo';
        this._hud.updateSourceBadge('demo', null);
        this._hud.setScenarioLocked(false);
        this._hud.setWaitBanner(false, '');
      };
      this._ws.onerror = () => { if (onFail) onFail(); };
    } catch {
      if (onFail) onFail();
    }
  }
  ...
  _emptyWaitData() {
    return {
      ...
      vital_signs: {},
      signal_field: { grid_size: [20, 1, 20], values: new Array(400).fill(0.02) },
      estimated_persons: 0,
      persons: [],
    };
  }
  ...
  _animate() {
    ...
    const isLiveCsi = this.settings.dataSource === 'ws' && this._liveData
      && this._liveData.type === 'sensing_update';
    const isWaitCsi = this.settings.dataSource === 'ws' && !isLiveCsi;

    if (isLiveCsi) {
      this._currentData = this._liveData;
    } else if (isWaitCsi) {
      this._currentData = this._emptyWaitData();
    } else {
    ...
    if (isLiveCsi || isWaitCsi) {
      for (const fig of this._figurePool.figures) {
        fig.group.visible = false;
      }
      this._presenceCartography.group.visible = true;
      this._presenceCartography.update(dt, elapsed, data);
      if (isWaitCsi) {
        this._hud.updateSourceBadge('wait', this._ws);
      }
    } else {
