...
export class HudController {
  ...
  initSettings() {
    ...
    document.getElementById('btn-export-settings').addEventListener('click', () => {
      ...
      a.download = 'lamarr-observatory-settings.json';
      a.click();
    });
  ...
  saveSettings() {
    try {
      localStorage.setItem('lamarr-observatory-settings', JSON.stringify(this._obs.settings));
    } catch {}
  }
  ...
  updateSourceBadge(dataSource, ws) {
    ...
    if (dataSource === 'live') {
      dot.className = 'dot dot--live'; label.textContent = 'CSI TRỰC TIẾP';
    } else if (dataSource === 'wait') {
      dot.className = 'dot dot--wait'; label.textContent = 'CHỜ CSI';
    } else {
      dot.className = 'dot dot--demo'; label.textContent = 'MÔ PHỎNG';
    }
  }

  setWaitBanner(show, text) {
    let el = document.getElementById('csi-wait-banner');
    if (!el) {
      el = document.createElement('div');
      el.id = 'csi-wait-banner';
      el.className = 'csi-wait-banner';
  ...
  setScenarioLocked(locked) {
    const sel = document.getElementById('scenario-quick-select');
    if (sel) {
      sel.disabled = locked;
      sel.title = locked ? 'Tắt khi chờ CSI thật' : 'Chọn kịch bản mô phỏng';
    }
  }
  ...
  updateHUD(data, demoData) {
    ...
    if (presEl) {
      ...
      if (ml === 'active') { presEl.classList.add('presence--active'); presLabel.textContent = 'CHUYỂN ĐỘNG'; }
      else if (cls.presence) { presEl.classList.add('presence--present'); presLabel.textContent = 'CÓ NGƯỜI'; }
      else { presEl.classList.add('presence--absent'); presLabel.textContent = 'VẮNG'; }
    }
