// LAMARR Application — Main Entry Point

import { TabManager } from './components/TabManager.js';
import { DashboardTab } from './components/DashboardTab.js';
import { HardwareTab } from './components/HardwareTab.js';
import { LamarrLiveTab } from './components/LamarrLiveTab.js';
...
class LamarrApp {
  ...
  async init() {
    try {
      console.log('Initializing LAMARR UI...');
      ...
      console.log('LAMARR UI initialized successfully');
  ...
  async initializeServices() {
    ...
    if (useMock) {
      ...
      try {
        ...
        this.showBackendStatus('Đã kết nối sensing-server LAMARR', 'success');
      } catch (error) {
        console.warn('⚠️ Backend not available:', error.message);
        this.showBackendStatus('Chưa có server — chạy sensing-server', 'warning');
      }
  ...
  initializeTabComponents() {
    ...
    if (demoContainer) {
      this.components.demo = new LamarrLiveTab(demoContainer);
  ...
  handleTabChange(newTab, oldTab) {
    ...
    if (oldTab === 'demo' && this.components.demo?.stop) {
      this.components.demo.stop();
    }
...
document.addEventListener('DOMContentLoaded', () => {
  window.lamarrApp = new LamarrApp();
  window.lamarrApp.init();
});
